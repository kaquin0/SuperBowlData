...
INSERT INTO superbowls(Date,SB,Winner,Winner_Pts,Loser,Loser_Pts,MVP,Stadium,City,State) VALUES ('Feb 13 2022','LV (56)','Los Angeles Rams',23,'Cincinnati Bengals',20,'Cooper Kupp','SoFi  Stadium','Ingelwood','California');
INSERT INTO superbowls(Date,SB,Winner,Winner_Pts,Loser,Loser_Pts,MVP,Stadium,City,State) VALUES ('Feb 7 2021','LV (55)','Tampa Bay Buccaneers',31,'Kansas City Chiefs',9,'Tom Brady','Raymond James Stadium','Tampa Bay','Florida');
...
/*Average Points Scored From All Super Bowl Winners */
SELECT AVG(Winner_Pts) FROM superbowls;

/*Teams with 4 or More Super Bowl Wins */
SELECT Winner AS Most_Super_Bowl_Wins FROM superbowls GROUP BY Winner HAVING COUNT(*) >= 4;

/*Super Bowl Losers that Scored 9 or less Points*/
SELECT COUNT(*) FROM superbowls WHERE Loser_Pts >= 0 AND Loser_Pts <= 9; 

/*Showing the number of Super Bowl Wins of the Top Five Teams */
SELECT COUNT(*),
    CASE
        WHEN Winner = 'San Francisco 49ers' THEN 'San Francisco 49ers'
        WHEN Winner = 'Dallas Cowboys' THEN 'Dallas Cowboys'
        WHEN Winner = 'Pittsburgh Steelers' THEN 'Pittsburgh Steelers'
        WHEN Winner = 'New England Patriots' THEN 'New England Patriots'
        WHEN Winner = 'New York Giants' THEN "New York Giants"
        ELSE 'Teams with Less Than 4 Wins'
